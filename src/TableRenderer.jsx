...
export default class TableRenderer extends Component {
  render() {
    ...
    <table className='pvtTable'>
      <thead>
        ...
        <tr>
          ...
          {(j == 0) &&
            ...
            rowSpan={colAttrs.length + (rowAttrs.length == 0 ? 0 : 1)}>Totals</th>
          }
        </tr>
        )})}
        ...
        <tr>
          {rowKey.map(function(txt,j){
            ...
            return <th className='pvtRowLabel'
              rowSpan={x} colSpan={j==rowAttrs.length-1 && colAttrs.length!=0 ? 2 : 1}>{txt}</th>
          })}
      ...
      <tr>
        <th className='pvtTotalLabel'
          colSpan={rowAttrs.length + (colAttrs.length == 0 ? 0 : 1)}>Totals</th>
